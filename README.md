...
./yuho xbrl-fact-store  # XBRL Fact Store生成
./yuho web          # ローカルWebアプリ起動
./yuho stock-status # 月次株価取得の状態確認
./yuho stock-refresh --force  # 月次株価を手動取得
./yuho cli build-ai-bundle  # AI投入用フォルダ生成
./yuho cli build-algorithm-audit  # アルゴリズム監査パック生成
./yuho cli extract-from-xbrl-fact-store  # Fact StoreからXBRL抽出候補を生成
./yuho cli compare-xbrl-fact-store  # 既存抽出とFact Store抽出を比較
...
別端末やTailscale/LAN越しに見る場合は、共有用のバックグラウンド起動を使います。
...
`web-start` は `0.0.0.0:8765` で待ち受け、接続先URLとログ場所を表示します。起動中は `caffeinate` でMacのスリープを抑止します。

現在のTailscale固定URL:

```text
http://business-dashboard.tail85c1ee.ts.net:8765/
```

同じTailscaleアカウントに接続したiPhone/Windows/Macから開けます。自宅LAN内だけで使う場合は `web-status` に表示される `LAN:` のURLも使えます。停止する場合:

```bash
./yuho web-stop
```

Tailscale URLが表示されない場合は、macOSのTailscaleアプリ、またはこのプロジェクト用のTailscaleデーモンが起動・ログイン済みか確認してください。
...
- 株価月次データの自動取得状態確認と手動取得
- `株価` タブで月次株価テーブルを閲覧
- `グラフ` タブでデータ種別を `株価月次` に切り替え、調整後終値・出来高・月次リターンをグラフ化
...
レビュー由来ルールが増えた場合は、定期的に `アルゴリズム監査パック生成` または `./yuho cli build-algorithm-audit` を実行します。生成先は `data/algorithm_audit/` です。`CODEX_MAINTENANCE_PROMPT.md` または互換用の `ALGORITHM_AUDIT_PROMPT.md` と同梱ファイルをCodex/AIに渡し、`review_*` セクションの肥大化、証跡1件の全社共通化、XBRL候補や同義語の広がりすぎ、低カバレッジ項目を監査します。通常リスクの改善はCodex/AIが実装・テスト・差分説明まで行い、ユーザーにコード妥当性の確認を求めません。確認が必要なのは、業務上の採用判断、破壊的なデータ削除、外部API・課金・公開範囲に影響する変更だけです。`config/automation.yml` の `review_gate.require_algorithm_audit` と `algorithm_audit_max_age_days` により、監査パックが未生成または期限切れの場合は年次自動取得ゲートで停止します。

## 株価月次データ

Webアプリ起動中は、`config/automation.yml` の `stock_price_monthly` 設定に従い、毎月指定日以降に前月末までの株価を自動チェックします。保存先:

```text
data/marts/market/security_master.csv
data/marts/market/stock_price_monthly.csv
data/marts/market/stock_price_monthly.parquet
data/raw/market/yahoo_chart/
data/automation/stock_monthly_last.json
```

取得する主な列は、月末日、証券コード、ティッカー、月次OHLC、調整後終値、出来高、月次リターン、配当、分割係数です。時価総額・発行済株式数は列だけ用意し、別ソースを追加した時点で計算します。非上場会社や証券コードがない会社は自動的に対象外です。

取得対象銘柄は `config/security_master.csv` で管理します。`listed_from` / `listed_to` が入っている銘柄は、その上場期間内だけ月次株価を取得します。たとえば前田建設工業は `2021-09-29` まで、三井住友建設は `2025-12-19` までの履歴銘柄として扱い、それ以降の毎月取得では自動的にスキップします。上場廃止月は月次系列として残せるよう、取得範囲は上場廃止日の属する月末までに切り詰めます。

手動確認・取得:

```bash
./yuho stock-status
./yuho stock-refresh --dry-run
./yuho stock-refresh --force
```

Web画面では `株価` タブでテーブル閲覧できます。グラフ化する場合は `グラフ` タブの上部 `データ` で `株価月次` を選び、会社・月・項目を選択します。初期表示は直近24か月の `調整後終値` です。

現時点の無料データ取得元は Yahoo Finance の chart API です。レスポンス原本は `data/raw/market/yahoo_chart/` に保存し、将来JPX/有料データベンダーへ差し替えられるよう、正本CSVには `source` と `source_symbol` を記録します。

Yahoo Finance は上場廃止銘柄の履歴を返さない場合があります。前田建設工業や三井住友建設のように親会社化・上場廃止が絡む銘柄は、J-Quantsなど公式系ソース、または手動バックフィル用CSVを追加して補完する前提です。上場廃止後の自動更新対象には含めません。

## XBRL Fact Store

XBRL抽出アルゴリズムを継ぎ足し型にしないため、EDINETの `csv.zip` を一度 `data/marts/xbrl_fact_store/` に正規化できます。正本は `facts.csv` / `facts.parquet`、Codex監査用の読み物は `document_digest.md` です。Markdownは確認用であり、抽出の正本にはしません。

生成:

```bash
./yuho xbrl-fact-store
./yuho xbrl-fact-store --doc-id S1007NWH
```

Fact Storeから既存形式の抽出候補を生成:

```bash
./yuho cli extract-from-xbrl-fact-store
```

既存抽出との差分確認:

```bash
./yuho cli compare-xbrl-fact-store
```

比較結果は `data/reports/xbrl_fact_store_compare.csv` に出力します。初期段階では `run-local` のデフォルト抽出には接続せず、比較レポートで一致率と差分理由を確認してから項目単位で移行します。
