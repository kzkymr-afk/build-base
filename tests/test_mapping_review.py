...
class ConfirmRejectMappingProposalTests(unittest.TestCase):
    ...
    def test_bulk_reject_conflicting_proposals_treats_zero_match_rate_as_conflict(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            conn = semantics_store.connect(root)
            ...
            result = mapping_review.bulk_reject_conflicting_proposals(root, reviewer="tester")

            self.assertEqual(result["candidates"], 1)
            self.assertEqual(result["rejected"], 1)
            self.assertEqual(result["examples"][0]["match_rate"], 0.0)
            conn = semantics_store.connect(root)
            row = conn.execute("select status, decided_by, evidence_json from concept_mappings where mapping_id='cmap_conflict'").fetchone()
            conn.close()
            self.assertEqual(row["status"], "rejected")
            self.assertEqual(row["decided_by"], "ai:model-x+auto_reject_conflict")
            evidence = json.loads(row["evidence_json"])
